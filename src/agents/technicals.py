...
# 在文件开头添加DIRECT_CURRENCIES常量
DIRECT_CURRENCIES = ['EUR', 'GBP', 'AUD', 'NZD']

##### Technical Analyst #####
def technical_analyst_agent(state: AgentState):
    """
    Sophisticated technical analysis system for forex markets that combines multiple trading strategies:
    1. Trend Following
    2. Mean Reversion
    3. Momentum
...
def calculate_mean_reversion_signals(prices_df):
    ...
    extreme_z_score = abs(z_score.iloc[-1]) > 1.5
...
def calculate_volatility_signals(prices_df):
    ...
    if current_vol_regime < 0.9 and vol_z < -0.8:
        signal = 'bullish'  # Low vol regime, potential for expansion
        confidence = min(abs(vol_z) / 3, 1.0)
    elif current_vol_regime > 1.1 and vol_z > 0.8:
